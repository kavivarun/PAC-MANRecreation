...
public class GhostStateManagerL2 : MonoBehaviour
{
    public enum GhostState2 { Spawn, Normal, Dead }

    [Header("Spawn/Exit Tilemaps")]
    private string spawnTilemapName;
    private string exitTilemapName;
    private bool startInSpawn = true;

    [Header("Timing & Scoring")]
    [SerializeField] private int ghostLife = 1;
    private int ghostKillPoints = 300;
    ...
    public GhostState2 CurrentState { get; private set; } = GhostState2.Normal;
    ...
    {
        if (isFrozen || CurrentState == GhostState2.Dead) return;
        CurrentState = GhostState2.Dead;
        visuals.EnterDead(true);
        Level2Manager.I?.AddScore(ghostKillPoints);
    ...
    void UpdateCanvas()
    {
        var canvas = transform.Find("GhostHealthCanvas");
        if (canvas == null)
        {
            return;
        }
